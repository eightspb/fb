...
function Backup-Database {
    ...
    $dbRunning = & $SshPath $Server "cd $RemotePath && docker compose -f $ComposeFile ps --status running 2>/dev/null | grep -q supabase && echo 'YES' || echo 'NO'"
    ...
    & $SshPath $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T supabase pg_dump -U postgres -d postgres --clean --if-exists" | Out-File -FilePath $backupFile -Encoding UTF8
...
function Apply-Migrations {
    ...
    & $SshPath $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T supabase psql -U postgres -d postgres -c `"$createTable`""
    ...
    foreach ($migrationPath in $migrations -split "`n") {
        ...
        $applied = & $SshPath $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T supabase psql -U postgres -d postgres -tAc `"SELECT EXISTS (SELECT 1 FROM schema_migrations WHERE name = '$migrationName');`""
        
        if ($applied -match "t") {
            Write-Info "  [SKIP] $migrationName (уже применена)"
        } else {
            Write-Info "  [APPLY] $migrationName"
            & $SshPath $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T supabase psql -U postgres -d postgres < $migrationPath"
            & $SshPath $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T supabase psql -U postgres -d postgres -c `"INSERT INTO schema_migrations (name) VALUES ('$migrationName');`""
        }
    }
...
function Show-Logs {
    ...
    & $SshPath $Server "cd $RemotePath && docker compose -f $ComposeFile logs --tail=20 nextjs-app 2>/dev/null || true"
}
