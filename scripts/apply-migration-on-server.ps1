...
ssh -p $SshPort $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T postgres psql -U postgres -d postgres -tA -c ""SELECT name FROM schema_migrations WHERE name = '006_fix_app_logs_rls';"""
...
if ($migrationExists) {
    ...
    ssh -p $SshPort $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T postgres psql -U postgres -d postgres -v ON_ERROR_STOP=1 -f migrations/006_fix_app_logs_rls.sql"
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "   ✅ Миграция применена успешно" -ForegroundColor Green
        
        # Добавляем запись в schema_migrations
        ssh -p $SshPort $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T postgres psql -U postgres -d postgres -c ""INSERT INTO schema_migrations (name) VALUES ('006_fix_app_logs_rls');"""
    } else {
...
ssh -p $SshPort $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T postgres psql -U postgres -d postgres -c ""SELECT relname, relrowsecurity FROM pg_class WHERE relname = 'app_logs';"""

# Проверяем политики RLS
Write-Host "`n4. Проверка политик RLS..." -ForegroundColor Yellow  
ssh -p $SshPort $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T postgres psql -U postgres -d postgres -c ""SELECT * FROM pg_policies WHERE tablename = 'app_logs';"""

# Тестируем SELECT запрос
Write-Host "`n5. Тест SELECT запроса (получение последних 3 логов)..." -ForegroundColor Yellow
ssh -p $SshPort $Server "cd $RemotePath && docker compose -f $ComposeFile exec -T postgres psql -U postgres -d postgres -c ""SELECT id, level, LEFT(message, 50) as message_preview, created_at FROM app_logs ORDER BY created_at DESC LIMIT 3;"""
