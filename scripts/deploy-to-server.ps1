...
function Backup-Database {
    ...
    $dbContainer = ssh $Server "cd $RemotePath && docker compose -f $ComposeFile ps -q postgres 2>/dev/null || echo 'fb-net-db'"
...
function Test-MigrationApplied {
    ...
    $tableExists = ssh $Server "cd $RemotePath && docker exec `$(docker compose -f $ComposeFile ps -q postgres) psql -U postgres -d postgres -tAc \"SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'schema_migrations');\""
...
"@
        ssh $Server "cd $RemotePath && docker exec -i `$(docker compose -f $ComposeFile ps -q postgres) psql -U postgres -d postgres" | Out-String -InputObject $createTable
    }
    
    # Проверяем, применена ли миграция
    $applied = ssh $Server "cd $RemotePath && docker exec `$(docker compose -f $ComposeFile ps -q postgres) psql -U postgres -d postgres -tAc \"SELECT EXISTS (SELECT 1 FROM schema_migrations WHERE name = '$migrationName');\""
...
function Apply-Migration {
    ...
    ssh $Server "cd $RemotePath && docker exec -i `$(docker compose -f $ComposeFile ps -q postgres) psql -U postgres -d postgres" | Out-String -InputObject $fullCommand
...
function Main {
    ...
    if (-not (Restart-Containers)) {
        ...
        Write-Warning "  Get-Content $BackupDir\db_backup_${Timestamp}.sql | ssh $Server 'cd $RemotePath && docker exec -i `$(docker compose -f $ComposeFile ps -q postgres) psql -U postgres -d postgres'"
        exit 1
    }
